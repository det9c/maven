...
for ipic in range(0,k):
    ...
    pred2=np.argwhere(pred.cpu().data.numpy()>box_tol)
    imap=indtrain[ipic]
'''
...
k=len(xtrain_tch)
box_tol=0.4
for ipic in range(0,k,20):
    pred = model(xtrain_tch[ipic:ipic+1].permute(0,3 , 1, 2))#.to(device))
    pred_sq=pred.view(10,10,5).detach().numpy()
